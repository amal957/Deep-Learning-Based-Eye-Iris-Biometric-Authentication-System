...
class IrisProcessor:
    def __init__(self, base_dir="iris_database", mask_model_path="iris_mask_model"):
        ...
        # Load the TensorFlow mask prediction model
        self.mask_model = self.load_mask_model()
    
    def load_mask_model(self):
        """Load the saved TensorFlow model for mask prediction"""
        try:
            model = tf.keras.models.load_model(self.mask_model_path)
            return model
        except Exception as e:
            st.error(f"Failed to load mask prediction model: {e}")
            return None
    
    def generate_mask(self, image_path):
        """Generate an iris mask using the pretrained TensorFlow model"""
        try:
            # Read the image
            ...
            # Resize to 128x128 as specified for model input
            resized_img = cv2.resize(img, (128, 128))
            
            # Preprocess for model input (normalize to [0,1])
            input_img = resized_img / 255.0
            input_img = np.expand_dims(input_img, axis=0)  # Add batch dimension
            
            # Predict mask
            predicted_mask = self.mask_model.predict(input_img)
            
            # Convert predicted mask to binary (0 or 255)
            binary_mask = (predicted_mask[0] > 0.5).astype(np.uint8) * 255
            
            # Resize back to original image size
            original_size_mask = cv2.resize(binary_mask, (img.shape[1], img.shape[0]))
            
            return original_size_mask, None
        except Exception as e:
            return None, f"Mask generation failed: {str(e)}"
    ...
    def preprocess_iris(self, image_path):
        ...
        # Generate mask using the TensorFlow model
        mask, error = self.generate_mask(image_path)
        if error:
            return None, error
    ...
    def enroll_user(self, user_id, name, iris_image_path, use_encryption=True):
        """Enroll a new user in the system"""
        # Create user directory
        user_dir = os.path.join(self.users_features_dir, user_id)
        os.makedirs(user_dir, exist_ok=True)
        
        # Generate mask and preprocess iris
        mask, error = self.generate_mask(iris_image_path)
        if error:
            return False, error
            
        # Save generated mask
        mask_path = os.path.join(user_dir, 'generated_mask.png')
        cv2.imwrite(mask_path, mask)
        
        # Preprocess iris with the generated mask
        unwrapped, error = self.preprocess_iris(iris_image_path)
        if error:
            return False, error
    ...
    def verify_user(self, iris_image_path, threshold=0.85):
...
def main():
    ...
    processor = IrisProcessor(mask_model_path="ensemble.h5")
    ...
    with tab1:
        ...
        with col1:
            ...
            if enroll_button and user_id and name and iris_file:
                ...
                # Process enrollment
                with st.spinner("Processing iris, generating mask, and enrolling user..."):
                    success, result = processor.enroll_user(user_id, name, iris_path, use_encryption)
                ...
                if success:
                    ...
                    # Display generated mask if available
                    mask_path = os.path.join(processor.users_features_dir, user_id, 'generated_mask.png')
                    if os.path.exists(mask_path):
                        col2.subheader("Generated Iris Mask")
                        col2.image(mask_path, caption="Auto-generated Iris Mask", use_column_width=True)
                else:
    ...
    with tab2:
        ...
        with col1:
            ...
            threshold = st.slider("Matching Threshold", min_value=0.5, max_value=1.0, value=0.75, step=0.01)
            
            verify_button = st.button("Verify Identity")
            
            if verify_button and verify_iris_file:
                # Save uploaded file temporarily
                with tempfile.NamedTemporaryFile(delete=False, suffix='.png') as tmp_iris:
                ...
                with st.spinner("Generating mask and verifying identity..."):
                    # First generate and display the mask
                    mask, mask_error = processor.generate_mask(iris_path)
                    if not mask_error and mask is not None:
                        # Save mask temporarily for display
                        mask_temp_path = iris_path + "_mask.png"
                        cv2.imwrite(mask_temp_path, mask)
                        col2.subheader("Generated Iris Mask")
                        col2.image(mask_temp_path, caption="Auto-generated Iris Mask", use_column_width=True)
                        os.unlink(mask_temp_path)
                    
                    # Then verify the identity
                    matched_user, similarities = processor.verify_user(iris_path, threshold)
